...
jcore92 - Lead Programmer/MutagenesisX Author" | zenity --text-info --title="About $app_name" --width=450 --height=550 #--ok-label="" --cancel-label="" &
...
jcore92 - Lead Programmer/MutagenesisX Author" | center
    ...
    if [ "$gui_flag" == "1" ]; then
        ...
        if zenity --text-info --title="$app_name: EULA" --width=550 --height=550 --filename="$APPDIR/LICENSE" --ok-label="I Agree"; then
            # User clicked "I Agree"
            text_delay
        else
            # User canceled or closed
    ...
    xprobe () {
        ...
        if [ "$XDG_SESSION_TYPE" = "tty" ]; then
            ...
            text_delay ; echo " ✕ TTY is not supported. Please use a Desktop Environment to open $app_name." | if [ "$gui_flag" = "1" ]; then
                zenity --text-info --width=450 --height=550
            else
                cat
                entertocontinue
            fi
        ...
        if grep -qi "ubuntu" /etc/os-release; then
            ...
            text_delay ; echo " ✕ Neither Ubuntu nor Debian base found." | if [ "$gui_flag" = "1" ]; then
                zenity --text-info --width=450 --height=550 --title="$app_name: $probe_name Report" --cancel-label=""
            else
                cat | print_red
            fi
...
'${missing[*]}'
" | if [ "$gui_flag" = "1" ]; then
                zenity --text-info --timeout=3 --width=450 --height=550 --title="$app_name: $probe_name Notification" --ok-label="" --cancel-label=""
...
Select an action to run:" | center
            ...
            if [ "$gui_flag" = "1" ]; then
                ...
                selection=$(zenity --list --column="Options" "${script_file_names[@]}" --width=450 --height=550 --title="$app_name: $selected" --ok-label="" --cancel-label="")
                exit_code=$?
            else
...
Select a menu option:" | center
            ...
            if [ "$gui_flag" = "1" ]; then
                #echo "${menu[@]}" | zenity --text-info --width=600 --height=800 --title="$app_name: $mainmenuname" --cancel-label=""
                selected=$(zenity --list --column="Options" "${menu[@]}" --width=450 --height=550 --title="$app_name: $mainmenuname" --ok-label="" --cancel-label="")
                exit_code=$?
                if [ "$exit_code" == "1" ]; then
                loop="0"
                break 2
...
if [ "$gui_flag" = "1" ]; then
    ...
    echo "100" >&3
    exec 3>&-  # Close fd
    printf '%s\n' "${xprob_messages[@]}" | zenity --text-info --title="$app_name: $probe_name Report" --timeout=3 --width=450 --height=550 #--ok-label="" --cancel-label=""
    #exit
else
    xprobe
fi
